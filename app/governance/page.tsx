...
export default function GovernanceDashboardPage() {
  ...
  if (status === 'unauthenticated' || !session || (session.user as any)?.roleName !== 'Governance') {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-red-100 text-red-700">
        <p className="text-xl">Akses ditolak. Anda tidak memiliki izin Governance atau belum login.</p>
  ...
  const currentRole = (session.user as any).roleName as 'Governance'; // Casting untuk memastikan tipe
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-6">Dashboard Governance</h1>
          <p className="text-lg text-gray-700 mb-4">
            Selamat datang, **{session.user.name || session.user.email}**!
            Ini adalah dashboard khusus untuk role **Governance**.
          </p>
          <div className="mt-8">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">Data Pasien & Fasilitas</h2>
            <p>Informasi detail tentang nakes, burn out, dan pengelolaan fasilitas rumah sakit.</p>
          </div>
          <div className="mt-8">
            <LogoutButton />
          </div>
